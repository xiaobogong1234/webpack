...
module.exports = angular.module('directive.bsTableControl', []).directive('bsTableControl', ['$compile', function($compile) {
        ...
        link: function($s, $el) {
            ...
            $s.$watch('bsTableControl.options', function(options) {
                ...
                if ($s.bsTableControl) {
                    ...
                    if ('searchHasFocus' in state) $el.closest(CONTAINER_SELECTOR).find(SEARCH_SELECTOR).focus();
                }
                // $el gets detached so have to recompute whole chain
            }, true);
            $s.$watch('bsTableControl.state', function(state) {
                if (!state && $s.bsTableControl) state = $s.bsTableControl.state = {};
